...
{% code title="session.tsx" overflow="wrap" lineNumbers="true" %}
```typescript
  ...
  const SessionInfo = () => {
    ...
    return (
      <div>
        ...
        {session
          ? <p>A mintbase session is active for {session.accountId}, created {session.createdAt} with token {token}</p>
